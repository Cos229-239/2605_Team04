...
export const useVaultStore = create<VaultState>((set, get) => ({
  ...
  removeChild: childId => {
    set(state => ({
      ...
      schedule: state.schedule.filter(item => item.childId !== childId),
      journal: state.journal.filter(entry => entry.childId !== childId)
    }));
    void persistVaultData(get());
  },
  ...
  removeScheduleItem: scheduleItemId => {
    set(state => ({ schedule: state.schedule.filter(item => item.id !== scheduleItemId) }));
    void persistVaultData(get());
  },
  ...
  loadOnboardingStatus: async () => {
    const [saved, savedThemeMode, savedScheduleBaseline, vaultData] = await Promise.all([
      AsyncStorage.getItem(ONBOARDING_COMPLETE_KEY),
      AsyncStorage.getItem(THEME_MODE_KEY),
      AsyncStorage.getItem(SCHEDULE_USER_BASELINE_KEY),
      loadVaultData()
    ]);
    const userOwnedVaultData = savedScheduleBaseline === 'true' ? vaultData : { ...vaultData, schedule: [] };
    if (savedScheduleBaseline !== 'true') {
      await Promise.all([
        persistVaultData(userOwnedVaultData),
    ...
    set({
      ...
      onboardingCompleted: saved === 'true' && userOwnedVaultData.children.length > 0,
